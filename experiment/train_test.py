...
def train(model, classifier, trainloader, valloader, cfg, logger, device):
    ...
    for e in range(cfg.epochs):
        if steps_since_update >= 200:
            logger.info("Early stopping triggered at epoch {}, global_step {}.".format(e, global_step))
            break
        running_loss = 0.0
        for i, samp in enumerate(trainloader):
            ...
            if global_step % cfg.print_interval == cfg.print_interval - 1:
                ...
                wandb.log({"running_loss": running_loss/cfg.print_interval})
                running_loss = 0.0
            ...
            if cfg.val_interval and global_step % cfg.val_interval == cfg.val_interval - 1:
                ...
                wandb.log({"h-precision": harsh_metrics_val["precision"],"h-recall": harsh_metrics_val["recall"],"h-f1": harsh_metrics_val["f1"],"h-rval": harsh_metrics_val["rval"],"l-precision": lenient_metrics_val["precision"],"l-recall": lenient_metrics_val["recall"],"l-f1": lenient_metrics_val["f1"],"l-rval": lenient_metrics_val["rval"]})
                if harsh_metrics_val["rval"] > best_rval:
                    ...
                    wandb.log({"best-h-rval": best_rval,"global_step": global_step})
